...
//unsigned long getNTime(String in) {
//  struct tm timeinfo;
//  if (!getLocalTime(&timeinfo)) {
//    Serial.println("Failed to obtain time");
//    return(0);
//  }
//  /*
//%A  Full weekday name ok
//%B  Full month name   ok
//%d  Day of the month ok
//%Y  Year ok
//%H  Hour in 24h format  ok
...
//    case "s":
//      char timeWeekDay[3];
//      strftime(timeWeekDay, 3, "%A", &timeinfo);
//      Serial.println(timeWeekDay);
//      break;
//    default:
//      // statements
...
void toNtp( void * pvParameters ) {
  while (1) {
    ...
    if (rtc.getEpoch() - lastepoc > 3600) {
      Serial.println("startwifi");
      Serial.println(getTimeepoc());
